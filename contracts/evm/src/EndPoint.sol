...
import {IntentStatus} from "./Enum.sol";
import "./Error.sol";
    ...
    ReentrancyGuard
{
    ...
    function _transferAsset(
        ...
        bool erc20TransferSuccess = IERC20(token).trySafeTransfer(
            receiver,
            amount
        );
    ...
    function proposeSettlement(
        ...
        if (!validators[msg.sender].active)
            revert ErrValidatorNotActive(msg.sender);
    ...
    function finalizeSettlement(
        ...
        IntentRecord storage record = intentRecords[intentHash];
        if (record.status != IntentStatus.PendingSettlement)
            revert ErrInvalidIntentStatus();
    ...
    function challengeSettlement(
        ...
        _slashValidator(
            pending.validator,
            validators[pending.validator].stake / 10
        );
        record.status = IntentStatus.Executed;
